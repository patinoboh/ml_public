...
def main(args: argparse.Namespace) -> float:
    ...
    bag_of_words = dict()
    nieco = [re.findall('\w+', i)for i in train_data]
    print(type(nieco[0][0]))
    for neviem_co in nieco:
        for term in neviem_co:
            if(term in bag_of_words):
                bag_of_words[term] += 1
            else:
                bag_of_words[term] = 1
    for term in list(bag_of_words):
        if(bag_of_words[term] <= 1):
            del bag_of_words[term]
    print(bag_of_words)

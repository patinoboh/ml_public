...
def main(args: argparse.Namespace) -> Optional[npt.ArrayLike]:
    ...
    def pooling(embeddings):
        t = 1
        return t * average_pooling(embeddings) + (1 - t) * max_pooling(embeddings)
    ...
    if args.predict is None:
        ...
        for review in train.data:
            review_embeddings = [word_embeddings[word] for word in review.split() if word in word_embeddings and word not in sklearn.feature_extraction._stop_words.ENGLISH_STOP_WORDS]
            train_as_vectors.append(pooling(review_embeddings)) 
        ...
        model = sklearn.neural_network.MLPClassifier(hidden_layer_sizes=2000, max_iter=100, alpha=0.0001, verbose=100)
        ...
        for review in test.data:
            review_embeddings = [word_embeddings[word] for word in review.split() if word in word_embeddings and word not in sklearn.feature_extraction._stop_words.ENGLISH_STOP_WORDS]
            test_as_vectors.append(pooling(review_embeddings)) 

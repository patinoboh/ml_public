...
parser.add_argument("--scaler", default="StandardScaler", type=str, help="Scaler to use")
        ...
        if args.model in ["most_frequent", "stratified"]:
            ...
            if args.model == "lr":
                model = [
                    ...
                    ("lr_cv", sklearn.linear_model.LogisticRegressionCV(Cs=np.geomspace(0.001, 100, 5), max_iter=100)),
                ]
            elif args.model == "lr3":
                model = [
                    ("poly", sklearn.preprocessing.PolynomialFeatures(3)),
                    ("lr_cv", sklearn.linear_model.LogisticRegression(C=1, max_iter=500)),
                ]
              ...
              model = [("lr", sklearn.linear_model.LogisticRegression())]
                ...
                model = [
                    ("MLP_ensemble", sklearn.ensemble.VotingClassifier([
                        ("MLP{}".format(i), sklearn.neural_network.MLPClassifier(tol=0, learning_rate_init=0.01, max_iter=200, hidden_layer_sizes=(500), activation="relu", solver="adam", verbose=1)) for i in range(3)])),
                ]
            ...
            model = sklearn.pipeline.Pipeline([
                ("preprocess", sklearn.compose.ColumnTransformer([
                    ...
                    ("scaler", getattr(sklearn.preprocessing, args.scaler)(), ~int_columns),
                ]))
            ] + model)

...
parser.add_argument("--dictionary", default=True, action="store_true", help="Use the dictionary")
...
class Dictionary:
    def __init__(self,
        ...
        if not os.path.exists(name):
            print("Downloading dictionary {}...".format(name), file=sys.stderr)
            urllib.request.urlretrieve(url + name, filename=name)
            urllib.request.urlretrieve(url + name.replace(".txt", ".LICENSE"), filename=name.replace(".txt", ".LICENSE"))

        # Load the dictionary to `variants` field.
        self.variants = {}
    ...
    if args.predict is None:
        ...
        if args.dictionary:
            ...
            if model_args.model_kind == "single":
                test_target = dict(zip(test_indices, model.predict_log_proba(test_data)))
            else:
                test_by_letter, target_by_letter, indices_by_letter, test_target = {}, {}, {}, {}
                for x, i in zip(test_data, test_indices):
                    test_by_letter.setdefault(x[0], []).append(x)
                    indices_by_letter.setdefault(x[0], []).append(i)
                for letter in test_by_letter:
                    log_probs = model[letter].predict_log_proba(test_by_letter[letter])
                    if len(model[letter].classes_) == 2:
                        log_probs = np.stack([
                            log_probs[:, int(i > 0)] if i in model[letter].classes_ else np.full(log_probs.shape[0], -np.inf) for i in range(3)
                        ], axis=1)
                    target_by_letter[letter] = list(log_probs)
                for x in test_data:
                    test_target[indices_by_letter[x[0]].pop(0)] = target_by_letter[x[0]].pop(0)
                ...
                for word in line.split(" "):
                    if word in dictionary.variants:
                        ...
                        for variant in dictionary.variants[word]:
                            score = sum(test_target[offset + i][test.letter_to_target(variant[i].lower(), model_args.target_mode)]
                        ...
                        for i in range(len(word)):
                            predictions[-1][-1].append(
                                test.target_to_letter(np.argmax(test_target[offset + i]), word[i].lower(), model_args.target_mode)

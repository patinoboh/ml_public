...
def main(args: argparse.Namespace) -> tuple[float, float]:
    ...
    rmses = []
    for l in lambdas:
        lr = sklearn.linear_model.Ridge(alpha=l).fit(train_data, train_target)
        predictions = lr.predict(test_data)
        rmses.append(sklearn.metrics.mean_squared_error(predictions, test_target, squared=False))
        if not best_rmse or rmses[-1] < best_rmse:
            best_lambda = l
            best_rmse = rmses[-1]

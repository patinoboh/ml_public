...
class DecisionTree:
    ...
    def _best_split(self, node):
        ...
        for feature in range(self._data.shape[1]):
            ...
            Gl, Hl, G, H = 0, 0, np.sum(self._gs[node.instances]), np.sum(self._hs[node.instances])
            for i in range(len(sorted_indices) - 1):
                Gl += self._gs[sorted_indices[i]]
                Hl += self._hs[sorted_indices[i]]
                ...
                criterion = -0.5 * Gl**2 / (self._l2 + Hl) + -0.5 * (G - Gl)**2 / (self._l2 + H - Hl)

...
def smo(
    ...
    for _ in range(args.max_iterations):
        ...
        for i, j in enumerate(generator.randint(len(a) - 1, size=len(a))):
            ...
            E_i = (a * train_target) @ K[i] + b - train_target[i]
            if not (a[i] < args.C - args.tolerance and train_target[i] * E_i < -args.tolerance) \
                    and not (a[i] > args.tolerance and train_target[i] * E_i > args.tolerance):
                continue
            ...
            E_j = (a * train_target) @ K[j] + b - train_target[j]
            new_a_j = a[j] - train_target[j] * (E_i - E_j) / eta
            ...
            b_i = b - E_i - train_target[i] * (new_a_i - a[i]) * K[i, i] - train_target[j] * (new_a_j - a[j]) * K[j, i]
            b_j = b - E_j - train_target[i] * (new_a_i - a[i]) * K[i, j] - train_target[j] * (new_a_j - a[j]) * K[j, j]

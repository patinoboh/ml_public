...
def main(args: argparse.Namespace) -> np.ndarray:
    ...
    for iteration in range(args.iterations):
        ...
        r = []
        for c in range(args.clusters):
            r.append(scipy.stats.multivariate_normal(mean=means[c], cov=covs[c]).pdf(data))
        r = np.array(r) * np.expand_dims(mixing_coefs, -1)
        r = r / np.sum(r, axis=0)

        # TODO: Update cluster `means`, `covs` and `mixing_coefs`.
        means = np.sum(np.expand_dims(data, 0) * np.expand_dims(r, -1), axis=1) / np.sum(r, axis=1, keepdims=True)
        covs = np.expand_dims(data, 0) - np.expand_dims(means, 1)
        covs = np.expand_dims(covs, -1) * np.expand_dims(covs, -2)
        covs = np.sum(covs * np.expand_dims(np.expand_dims(r, -1), -1), axis=1) / np.expand_dims(np.sum(r, axis=1, keepdims=True), -1)
        mixing_coefs = np.sum(r, axis=1) / np.sum(r)

        # TODO: Compute the negative log-likelihood of the current model to `loss`.
        loss = 0
        for c in range(args.clusters):
            loss += scipy.stats.multivariate_normal(mean=means[c], cov=covs[c]).pdf(data) * mixing_coefs[c]

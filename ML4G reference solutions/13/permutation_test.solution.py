...
def main(args: argparse.Namespace) -> float:
    ...
    predictions = np.stack([model.predict(test_data) for model in models], axis=1)
    for i in range(args.random_samples):
        assignments = generator.choice(2, size=len(test_data), replace=True)
        scores.append(100 * np.mean(test_target == predictions[indices, assignments]))

    # TODO: Store the performance of the `models[1]` as percents in `second_model`.
    second_model = 100 * np.mean(test_target == predictions[:, 1])
    ...
    p_value = 100 * np.mean(scores >= second_model)

...
def main(args: argparse.Namespace) -> Optional[npt.ArrayLike]:
    if args.predict is None:
        ...
        elif args.model.startswith("multinomial_nb"):
            words, estimator = 5000, sklearn.naive_bayes.MultinomialNB(alpha=0.25 if "chars" in args.features else 10)
        elif args.model == "complement_nb":
            words, estimator = 5000, sklearn.naive_bayes.ComplementNB(alpha=0.25 if "chars" in args.features else 10)
        elif args.model == "complement_nb_norm":
            words, estimator = 5000, sklearn.naive_bayes.ComplementNB(alpha=0.25 if "chars" in args.features else 10, norm=True)
        elif args.model == "gbt":
            words, estimator = 5000, sklearn.ensemble.GradientBoostingClassifier(n_estimators=100, max_depth=3, subsample=1, verbose=1)
        elif args.model == "lr":
        ...
        if args.model == "multinomial_nb_norm":
            estimator.feature_log_prob_ /= -np.sum(estimator.feature_log_prob_, axis=1, keepdims=True)
            estimator.class_log_prior_[:] = 0
